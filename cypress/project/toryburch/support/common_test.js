...
export function login(){
  cy.fixture("data").then(data=>{
    ...
    cy.get('.iconfont.iconuser-usage.gray50').click({ force: true });
    ...
    cy.get('.pd-l-10.gray50').should('contain','Hi')
  })
}
...
export function search(){
  cy.fixture("data").then(data=>{
    ...
    cy.get('.tab-item.search>span').click({ force: true });
...
//plp加购
export function plp_add_to_cart(){
  cy.allure().step('hover商品');
  cy.get('.imglist .self-swiper').realHover();
  cy.allure().step('点击plp加入购物车');
  cy.get('.selectwarp .btn.btn-medium.btn-bg').click({ force: true });
  cy.allure().step('验证加购成功');
  cy.get('div.shopping-cart-icon span.pd-r-4').last().should('have.text','1')  //last()指最后一个可用的，should('have.text','1')指断言判断文案是否为1
}

//导航
export function into_plp_from_navigation(){
  cy.allure().step('hover一级导航配件');
  cy.get('.common-header .nav-list>li:nth-child(5)').realHover();
  cy.allure().step('点击三级导航腰带');
  cy.contains('腰带').click();
...
export function pdp_add_to_cart(){
  ...
  cy.allure().step('选择黑色');
  cy.get('.pdpwarp .particulars .particulars-price-ui-4>div:nth-child(2)>img').click();
  cy.allure().step('验证是否选择黑色');
  cy.get('.pdpwarp .particulars .particulars-price-ui-5>span').should('have.text','午夜黑');
  cy.allure().step('点击请选择尺码');
  cy.get('.pdpwarp .particulars .select-text').click();
  cy.allure().step('点击XS');
  cy.get('.pdpwarp .particulars .el-select-dropdown__item:not(.is-disabled)').first().click();
  cy.allure().step('验证是否选择XS');
  cy.get('.pdpwarp .particulars .text').should('have.text','XS')
  cy.allure().step('点击加入购物车');
  cy.get('.pdpwarp .particulars .btn.btn-medium:nth-child(1)').click();
  cy.allure().step('验证加购成功');
  cy.get('div.shopping-cart-icon span.pd-r-4').last().should('have.text','1');
}

//pdp加购111111
export function pdp_add_to_cart1(){
  cy.visitUrl('/item/79436-909')
  cy.allure().step('选择黑色');
  cy.get('.pdpwarp .particulars .particulars-price-ui-4>div:nth-child(2)>img').click();
  cy.allure().step('验证是否选择黑色');
  ...
  cy.get('.pdpwarp .particulars .el-select-dropdown__item:not(.is-disabled)').first().click();
  cy.allure().step('验证是否选择XS');
  return get('.pdpwarp .particulars .text')
}
export function pdp_add_to_cart2(){
  cy.allure().step('点击加入购物车');
  cy.get('.pdpwarp .particulars .btn.btn-medium:nth-child(1)').click();
  cy.allure().step('验证加购成功');
  cy.get('div.shopping-cart-icon span.pd-r-4').last().should('have.text','1');
...
export function pdp_buy_now(){
  ...
  cy.get('.pdpwarp .particulars .btn.btn-medium:nth-child(2)').click();
}


//进入购物车
export function into_shoppingcart(){
  cy.allure().step('hover购物车icon');
  cy.get('.content-wrap .shopping-cart-icon .iconfont.iconbag-usage').realHover();
  cy.allure().step('点击前往购物袋');
  cy.get('.mini-cart-foot>a').click();
...
export function shoppingcart_to_checkout(){
  ...
  cy.get('.content-wrap .shopping-cart-icon .iconfont.iconbag-usage').realHover();
  cy.allure().step('点击立即购买');
  cy.get('.mini-cart-foot>button').click();
}
...
//会员结算页（提交订单）
export function checkout_member_part2(){
...
//commonUtil.checkout_page_default(pay[index].paymethod).then(obj => {
 ...
 // commonUtil.cancel_order(obj.text(), pay[index].paymethod);
...
export function visitor_cancel_order(code){
  cy.fixture("data").then(data=>{
    ...
    //(code.text().substring(5))是指显示所取值只取第6位开始的值,从0开始
...
export function test(){
  cy.allure().step('选择支付方式:微信');
  cy.visit('/checkout-pay/222070510165008905')
}
...
export function into_plp11(p_url){
  if(p_url){
    cy.visit(p_url);
  }
  else{
...
//会员结算页（支付宝）
export function checkout_member_alipay(){
  cy.wait(1000)
  cy.allure().step('验证地址是否存在');
  cy.get('.address-item.mg-b-30.active .name').should('contain','自动化测试');
  cy.allure().step('点击下一步');
...
//会员结算页（花呗）
export function checkout_member_huabei(){
  cy.wait(1000)
  cy.allure().step('验证地址是否存在');
  cy.get('.address-item.mg-b-30.active .name').should('contain','自动化测试');
  cy.allure().step('点击下一步');
...
//会员结算页（微信）
export function checkout_member_wapay(){
  cy.wait(1000)
  cy.allure().step('验证地址是否存在');
  cy.get('.address-item.mg-b-30.active .name').should('contain','自动化测试');
  cy.allure().step('点击下一步');
...
//游客结算页（默认支付方式支付宝）
export function checkout_visitor1(){
  cy.fixture("data").then(data=>{
    cy.wait(1000)
    cy.allure().step('输入姓名');
    cy.get('.name-phone.mg-b-30 .name-wrap').type(data.visitor.name);
