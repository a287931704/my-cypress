...
export function search_operate(){
  ...
  cy.allure().step('搜索制定商品"6146709PMA41000"');
  cy.get('#keyword').type('6146709PMA41000');
...
export function into_pdp_from_plp(){
  ...
  cy.get('.spice-product-tiles-slot.spice-product-index.grid-cell').first()
    .realHover();
  cy.get('.spice-item-grid-hover.spice-product-index-sub').contains('立即购买').click();
  //cy.get('.visual-hover-img').click()
  cy.get('.spice-style-number-title').should('contain','614670 9PMA4 1000')
}
