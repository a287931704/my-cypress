...
describe('VLTN-Mobile创单流程-'+Cypress.env('environment'), () => {
  beforeEach(() => {
  ...
  it('普通商品创单流程-'+Cypress.env('environment'), () => {
    ...
    cy.fixture('paymethod.json').then((pay) => {
      for (const index in pay) {
        ...
        //加购并进入结算页
        commonUtil.addcart_goto_checkout(1);
        //结算页默认操作并却结算
        cy.allure().step(pay[index].paymethod+'下单发起');
        commonUtil.checkout_page_default(pay[index].paymethod, 1).then(obj => {
          if(pay[index].paymethod==='花呗分期'){
            cy.wait(5000)
          }
          //取消订单，传入订单号和支付方式，支付方式将在银联支付跳转bug修复后删除此传参
          commonUtil.cancel_order(0, 1);
        })
      }
  ...
  it('定制商品创单流程-'+Cypress.env('environment'), () => {
    ...
    cy.fixture('paymethod.json').then((pay) => {
      if (Cypress.env('environment') != 'uat') {
        //从导航进入宠物定制商品PLP
        commonUtil.into_pets_plp(1);
        //在PLP默认选择第一个商品进入PDP
        commonUtil.into_pdp_from_plp(1);
        cy.wait(2000)
        //宠物定制商品操作并去结算
        commonUtil.vip_item_operate(1);
        //结算页面操作
        commonUtil.checkout_page_default(pay[0].paymethod,1).then(obj => {
          //取消订单
          commonUtil.cancel_order(obj.text(),1);
        })
      }else{
